...
std::vector<Airport> Graph::getReachableAirports(const std::string &start_airport_code, int num_flights) const {
    ...
    while (!q.empty()) {
        ...
        for (const Flight &flight : outgoing_flights) {
            ...
            if (flights_taken <= num_flights &&
                std::find_if(reachable_airports.begin(), reachable_airports.end(), [&](const Airport &a) { return a.getCode() == destination_code; }) == reachable_airports.end()){
